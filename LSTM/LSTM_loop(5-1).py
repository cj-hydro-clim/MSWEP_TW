# Add up to 4 adjacent grids as input at the same time. (5-1:16 combinations)
import pandas as pd
import numpy as np
import os
...
data = []
name = []
corr_num = np.array([range(1,17)], dtype='float64')
corr_num = corr_num.T
dafT = pd.read_csv(data_path+'test_input_5-1.csv',header=None,names=['year','month','day','1','2','3','4','5','TCCIP'])
data = np.array(dafT.iloc[0:1462,2:9])
test = np.array(dafT.iloc[1462:1828:,2:8])
tccip_2019 = np.array(dafT.iloc[1462:1827:,8:9])

# Data extraction
# Traning dataset
#------------------------------------------------------------------------------------------#
# The target grid is used as a separate input for calibration
data1	=	np.array(data[0:1462,[1	,6	]])

# The target grid adds 1 surrounding grid as input for correction at the same time.
data2	=	np.array(data[0:1462,[1,2	,6 	]])
data3	=	np.array(data[0:1462,[1,3	,6 	]])
data4	=	np.array(data[0:1462,[1,4	,6 	]])
data5	=	np.array(data[0:1462,[1,5	,6 	]])

# The target grid adds 2 surrounding grid as input for correction at the same time.
data6	=	np.array(data[0:1462,[1,2,3 ,6 	]])
data7   =	np.array(data[0:1462,[1,2,4	,6 	]])
data8   =	np.array(data[0:1462,[1,2,5	,6 	]])
data9   =	np.array(data[0:1462,[1,3,4	,6 	]])
data10  =	np.array(data[0:1462,[1,3,5	,6 	]])
data11  =	np.array(data[0:1462,[1,4,5	,6 	]])

# The target grid adds 3 surrounding grid as input for correction at the same time.
data12	=	np.array(data[0:1462,[1,2,3,4	,6	]])
data13	=	np.array(data[0:1462,[1,2,3,5	,6	]])
data14	=	np.array(data[0:1462,[1,2,4,5	,6	]])
data15	=	np.array(data[0:1462,[1,3,4,5	,6	]])

# The target grid adds 4 surrounding grid as input for correction at the same time.
data16	=	np.array(data[0:1462,[1,2,3,4,5 ,6	]])
#------------------------------------------------------------------------------------------#
...
test6	=	np.array(test[0:366,[1,2,3   	]])
test7   =	np.array(test[0:366,[1,2,4	 	]])
test8   =	np.array(test[0:366,[1,2,5	 	]])
test9   =	np.array(test[0:366,[1,3,4	 	]])
test10  =	np.array(test[0:366,[1,3,5	 	]])
test11  =	np.array(test[0:366,[1,4,5	 	]])

test12	=	np.array(test[0:366,[1,2,3,4		]])
test13	=	np.array(test[0:366,[1,2,3,5		]])
test14	=	np.array(test[0:366,[1,2,4,5	 	]])
test15	=	np.array(test[0:366,[1,3,4,5		]])

test16	=	np.array(test[0:366,[1,2,3,4,5  	]])
#------------------------------------------------------------------------------------------#

# Variable k is the file number 1~16.
# The variable j is entered and other parameters. If for file k=1,j=1. k=2~5,j=2. etc.
#    1    = 1 ( 1 )
# 2  ~  5 = 2 ( 4 )
# 5  ~ 11 = 3 ( 6 )
# 12 ~ 15 = 4 ( 4 )
...
all_pccs.to_csv('/Users/cjchen/Desktop/git-repos/MSWEP_TW/LSTM/output/'+'test_output_5-1_cc.csv',header=False, index=False)