# Add up to 2 adjacent grids as input at the same time. (3-1:4 combinations)
import pandas as pd
import numpy as np
import os
...
data = []
name = []
corr_num = np.array([range(1,5)], dtype='float64')
corr_num = corr_num.T
dafT = pd.read_csv(data_path+'test_input_3-1.csv',header=None,names=['year','month','day','1','2','3','TCCIP'])
data = np.array(dafT.iloc[0:1462,2:7])
test = np.array(dafT.iloc[1462:1828:,2:6])
tccip_2019 = np.array(dafT.iloc[1462:1827:,6:7])

# Data extraction
# Traning dataset
#------------------------------------------------------------------------------------------#
# The target grid is used as a separate input for calibration
data1	=	np.array(data[0:1462,[1	,4	]])

# The target grid adds 1 surrounding grid as input for correction at the same time.
data2	=	np.array(data[0:1462,[1,2	,4 	]])
data3	=	np.array(data[0:1462,[1,3	,4 	]])

# The target grid adds 2 surrounding grid as input for correction at the same time.
data4	=	np.array(data[0:1462,[1,2,3 ,4 	]])
#------------------------------------------------------------------------------------------#
...
test4	=	np.array(test[0:366,[1,2,3   	]])
#------------------------------------------------------------------------------------------#

# Variable k is the file number 1~4.
# The variable j is entered and other parameters. If for file k=1,j=1. k=2~3,j=2. etc.
#    1    = 1 ( 1 )
# 2  ~  3 = 2 ( 2 )
#    4    = 3 ( 1 )
for k in range(1,5):
...
all_pccs.to_csv('/Users/cjchen/Desktop/git-repos/MSWEP_TW/LSTM/output/'+'test_output_3-1_cc.csv',header=False, index=False)