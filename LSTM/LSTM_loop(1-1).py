# Only the target grid is used as input for calibration. (1-1)
import pandas as pd
import numpy as np
import os
...
data = []
name = []
corr_num = np.array([range(1,2)], dtype='float64')
corr_num = corr_num.T
dafT = pd.read_csv(data_path+'test_input_1-1.csv',header=None,names=['year','month','day','1','TCCIP'])
data = np.array(dafT.iloc[0:1462,2:5])
test = np.array(dafT.iloc[1462:1828:,2:4])
tccip_2019 = np.array(dafT.iloc[1462:1827:,4:5])

# Traning dataset
#------------------------------------------------------------------------------------------#
data1	=	np.array(data[0:1462,[1	,2	]])
#------------------------------------------------------------------------------------------#
...
# Variable k is the file number 1.
# The variable j is entered and other parameters. If for file k=1,j=1.
#    1    = 1 ( 1 )
k=1
j=1
...
all_pccs.to_csv('/Users/cjchen/Desktop/git-repos/MSWEP_TW/LSTM/output/'+'test_output_1-1_cc.csv',header=False, index=False)
