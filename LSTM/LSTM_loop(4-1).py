# Add up to 3 adjacent grids as input at the same time. (4-1:8 combinations)
import pandas as pd
import numpy as np
import os
...
data = []
name = []
corr_num = np.array([range(1,9)], dtype='float64')
corr_num = corr_num.T
dafT = pd.read_csv(data_path+'test_input_4-1.csv',header=None,names=['year','month','day','1','2','3','4','TCCIP'])
data = np.array(dafT.iloc[0:1462,2:8])
test = np.array(dafT.iloc[1462:1828:,2:7])
tccip_2019 = np.array(dafT.iloc[1462:1827:,7:8])

# Data extraction
# Traning dataset
#------------------------------------------------------------------------------------------#
# The target grid is used as a separate input for calibration
data1	=	np.array(data[0:1462,[1	,5	]])

# The target grid adds 1 surrounding grid as input for correction at the same time.
data2	=	np.array(data[0:1462,[1,2	,5 	]])
data3	=	np.array(data[0:1462,[1,3	,5 	]])
data4	=	np.array(data[0:1462,[1,4	,5 	]])

# The target grid adds 2 surrounding grid as input for correction at the same time.
data5	=	np.array(data[0:1462,[1,2,3 ,5 	]])
data6   =	np.array(data[0:1462,[1,2,4	,5 	]])
data7   =	np.array(data[0:1462,[1,3,4	,5 	]])

# The target grid adds 3 surrounding grid as input for correction at the same time.
data8 	=	np.array(data[0:1462,[1,2,3,4	,5	]])
#------------------------------------------------------------------------------------------#
...
test5	=	np.array(test[0:366,[1,2,3   	]])
test6   =	np.array(test[0:366,[1,2,4	 	]])
test7   =	np.array(test[0:366,[1,3,4	 	]])

test8 	=	np.array(test[0:366,[1,2,3,4		]])
#------------------------------------------------------------------------------------------#

# Variable k is the file number 1~8.
# The variable j is entered and other parameters. If for file k=1,j=1. k=2~4,j=2. etc.
#    1    = 1 ( 1 )
# 2  ~  4 = 2 ( 3 )
# 5  ~  7 = 3 ( 3 )
#    8    = 4 ( 1 )
...
all_pccs.to_csv('/Users/cjchen/Desktop/git-repos/MSWEP_TW/LSTM/output/'+'test_output_4-1_cc.csv',header=False, index=False)